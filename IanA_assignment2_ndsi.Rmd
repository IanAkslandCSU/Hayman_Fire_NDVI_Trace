...
```{r, echo = F}
ggplot_NDSI_cfs <- merge(x=q_wtr_yr_Mean,y=NDSI_CompleteMean,by="wtr_yr",all.x=TRUE)
  ggplot(data = ggplot_NDSI_cfs,aes(x = mean_ndsi,y = median_q, col=wtr_yr))+
  geom_point(size = 3)+
...
```{r, echo = F}
ggplot(data = ggplot_NDSI_cfs,aes(x = max_ndsi,y = median_q, col=wtr_yr))+
  geom_point(size = 3)+
    xlab('Max Snow Cover')+
    ylab('Median Discharge (cfs)')
```
